...
Purpose: This file will take dem tiles and assemble them back into a single image
*/

// prototypes and dependencies for this file
#include "gtopoAssemble.h"
...
int main(int argc, char **argv) {
    ...
    } else if ((argc-4) % 5 != 0) { // check there is the correct number of arguments: 4 + 5n 
        // adjust return code and go to end of function 
        return exit_code_with_message(ERROR_ARG_COUNT, NULL);
    }

    // check that argv[2], argv[3], are ints
...
int assembler(char **arguments, int arg_length) {
    ...
    for (int argument_index = 0; argument_index < total_tiles; argument_index ++) {
        // get data about the current tile fron the given arguments, example each row is at index 5(n+1) - 1
        char * current_filename = arguments[5 * (argument_index + 1) + 1];
        int row = atoi(arguments[5 * (argument_index + 1) - 1]); 
        int col = atoi(arguments[5 * (argument_index + 1)]);
	    int current_width = atoi(arguments[5 * (argument_index + 1) + 2]);
	    int current_height = atoi(arguments[5 * (argument_index + 1) + 3]);
        
        //printf("DETAILS:\n filename: %s\n row: %d\n col: %d\n width: %d\n height: %d\n", current_filename, row, col, current_width, current_height);
